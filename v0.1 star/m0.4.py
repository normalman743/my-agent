...
class UltraTechAIPet(QWidget):
    ...
    def __init__(self):
        ...
        # 改进的对话系统
        self.chat_bubbles = []  # 存储多个对话气泡
        self.input_bubble = None
        self.input_visible = False
        self.chat_history = []  # 聊天历史
        self.log_file = "starai.log"
    ...
    def save_chat_log(self, user_msg, ai_msg):
        """保存聊天记录到文件"""
        try:
            with open(self.log_file, 'a', encoding='utf-8') as f:
                timestamp = datetime.now().strftime('%H:%M:%S')
                f.write(f"[{timestamp}] 用户: {user_msg}\n")
    ...
    def mouseDoubleClickEvent(self, event):
        if event.button() == Qt.LeftButton:
            self.add_chat_bubble("科技问候！✨⚡", is_ai=True)
            event.accept()

    def contextMenuEvent(self, event):
        # 右键切换输入框显示状态
        if self.input_visible:
            self.hide_input_bubble()
        else:
    ...
    def show_input_bubble(self):
        ...
        self.input_bubble = InputBubble(self, parent=self)
        pet_pos = self.pos()
    ...
    def add_chat_bubble(self, message, is_ai=True):
        """添加对话气泡，支持多条显示"""
        # 创建新的对话气泡
        new_bubble = ChatBubble(message, self, is_ai=is_ai, parent=self)
        self.chat_bubbles.append(new_bubble)
        
        # 清理过期的气泡（保持最多5条）
        while len(self.chat_bubbles) > 5:
            old_bubble = self.chat_bubbles.pop(0)
            old_bubble.close()
        
        # 重新排列所有气泡位置
        self.arrange_chat_bubbles()
        
        # 显示新气泡
        new_bubble.show()

    def arrange_chat_bubbles(self):
        """重新排列对话气泡位置"""
        pet_pos = self.pos()
        base_x = pet_pos.x() + 100
        base_y = pet_pos.y() - 40
        
        # 从下往上排列气泡
        current_y = base_y
        for i, bubble in enumerate(reversed(self.chat_bubbles)):
            if bubble and not bubble.isHidden():
                bubble.move(base_x, current_y)
                current_y -= (bubble.height() + 10)  # 气泡间距10像素

    def on_chat_sent(self, user_msg, ai_msg):
        """处理聊天消息"""
        # 保存到聊天记录
        self.save_chat_log(user_msg, ai_msg)
        
        # 只显示AI回复
        self.add_chat_bubble(f"AI: {ai_msg}", is_ai=True)
        
        # 隐藏输入框
        self.hide_input_bubble()

class ChatBubble(QWidget):
    def __init__(self, message, parent_pet, is_ai=True, parent=None):
        ...
        # 自动计算宽高
        font = QFont("Menlo", 12, QFont.Bold)
    ...
    def fade_out(self):
        ...
        self.timer2.timeout.connect(self.reduce_opacity)
        self.timer2.start(50)  # 每50ms减少透明度
    ...
    def reduce_opacity(self):
        ...
        if self.opacity <= 0:
            ...
            self.close()
            # 从父级的气泡列表中移除
            if self in self.parent_pet.chat_bubbles:
                self.parent_pet.chat_bubbles.remove(self)
        self.update()
    ...
    def paintEvent(self, event):
        ...
        if self.is_ai:
            # AI消息使用蓝色主题
            painter.setBrush(QColor(40, 60, 80, int(220 * self.opacity)))
            painter.setPen(QPen(QColor(150, 200, 255, int(200 * self.opacity)), 2))
            text_color = QColor(220, 255, 255, int(255 * self.opacity))
        else:
            # 用户消息使用绿色主题
            painter.setBrush(QColor(40, 80, 40, int(220 * self.opacity)))
            painter.setPen(QPen(QColor(150, 255, 150, int(200 * self.opacity)), 2))
            text_color = QColor(220, 255, 220, int(255 * self.opacity))
...
class InputBubble(QWidget):
    def __init__(self, parent_pet, parent=None):
        super().__init__(parent)
        self.parent_pet = parent_pet
    ...
    def send_message(self):
        ...
        if text:
            ai_reply = self.fake_ai_reply(text)
            # 通知父级处理聊天
            self.parent_pet.on_chat_sent(text, ai_reply)
            self.input.clear()
    ...
    def fake_ai_reply(self, user_text):
        ...
        elif "再见" in user_text or "bye" in user_text.lower():
            return "再见！有需要随时叫我哦~ 👋"
        else:
            replies = [
                "我在这里，有什么可以帮你？",
    ...
    def focusOutEvent(self, event):
        """失去焦点时关闭输入框"""
        QTimer.singleShot(100, self.parent_pet.hide_input_bubble)
...
if _sys.platform == "darwin":
    ...
    from Foundation import NSObject, NSMakeRect, NSPoint, NSRect, YES, NO
    from AppKit import (
        NSApplication, NSWindow, NSView, NSWindowStyleMaskBorderless, NSBackingStoreBuffered,
        NSWindowCollectionBehaviorCanJoinAllSpaces, NSWindowCollectionBehaviorStationary,
        NSFloatingWindowLevel, NSColor, NSEvent, NSGraphicsContext, CGContextClearRect,
        CGContextSetRGBFillColor, CGContextFillEllipseInRect, CGContextSetRGBStrokeColor,
        CGContextSetLineWidth, CGContextStrokeEllipseInRect, CGContextMoveToPoint,
        CGContextAddLineToPoint, CGContextStrokePath
    )

    class NativeAIPetView(NSView):
        ...
        def update_animation(self):
            ...
            self.update_data_streams()
            self.update_arc_particles()
            self.set_needs_display_(True)

        def update_mouse_proximity(self):
            try:
                global_mouse = NSEvent.mouseLocation()
                pet_center = self.bounds().center()
                dx = global_mouse.x - pet_center.x
                dy = global_mouse.y - pet_center.y
                self.mouse_distance = math.sqrt(dx*dx + dy*dy)
                if self.mouse_distance < 200:
        ...
        def draw(self):
            ...
            self.draw_ultra_outer_effects(context, self.bounds().midX, self.bounds().midY)
            self.draw_tech_elements(context, self.bounds().midX, self.bounds().midY)
            self.draw_data_streams(context, self.bounds().midX, self.bounds().midY)
            self.draw_enhanced_orbit_particles(context, self.bounds().midX, self.bounds().midY)
            self.draw_arc_particles(context, self.bounds().midX, self.bounds().midY)
            self.draw_ultra_core(context, self.bounds().midX, self.bounds().midY)
            self.draw_inner_effects(context, self.bounds().midX, self.bounds().midY)
            self.draw_enhanced_eyes(context, self.bounds().midX, self.bounds().midY)
            self.draw_energy_waves(context, self.bounds().midX, self.bounds().midY)
            self.draw_lens_flare(context, self.bounds().midX, self.bounds().midY)

        def draw_ultra_outer_effects(self, context, x, y):
            """绘制超炫外围光环 - 8层强光"""
            base_intensity = self.intensity_boost
            for i in range(8):
                ring_radius = self.radius + 15 + i * 8
                ...
                g = int(200 + 55 * color_shift)
                b = 255
                CGContextSetRGBFillColor(context, r/255, g/255, b/255, alpha/255.0)
                CGContextFillEllipseInRect(context, NSMakeRect(x - ring_radius, y - ring_radius, ring_radius * 2, ring_radius * 2))
        def draw_tech_elements(self, context, x, y):
            """绘制科技元素"""
            # 1. 雷达扫描线
            scan_length = 80 * self.intensity_boost
            scan_end_x = x + scan_length * math.cos(self.scan_angle)
            scan_end_y = y + scan_length * math.sin(self.scan_angle)
            scan_alpha = int(200 * self.intensity_boost)
            CGContextSetRGBStrokeColor(context, 0, 1, 1, scan_alpha/255.0)
            CGContextSetLineWidth(context, 3)
            CGContextMoveToPoint(context, x, y)
            CGContextAddLineToPoint(context, scan_end_x, scan_end_y)
            ...
            for hex_elem in self.hex_grid:
                ...
                pulse = 0.5 + 0.5 * math.sin(hex_elem['pulse_phase'])
                alpha = int(hex_elem['alpha'] * 255 * pulse * self.intensity_boost)
                hex_color = QColor(0, 255, 200, alpha)
                self.draw_hexagon(context, hex_x, hex_y, hex_elem['size'], hex_color)
            # 3. 能量光束
            for beam in self.energy_beams:
                beam_start_x = x + 20 * math.cos(beam['angle'])
                beam_start_y = y + 20 * math.sin(beam['angle'])
                ...
                beam_gradient.setColorAt(0.5, QColor(100, 255, 255, alpha))
                beam_gradient.setColorAt(1, QColor(255, 255, 100, alpha//2))
                CGContextSetLineWidth(context, beam['width'])
                CGContextMoveToPoint(context, beam_start_x, beam_start_y)
                CGContextAddLineToPoint(context, beam_end_x, beam_end_y)
                CGContextStrokePath(context)

        def draw_hexagon(self, context, x, y, size, color):
            points = []
            for i in range(6):
                angle = i * math.pi / 3
                px = x + size * math.cos(angle)
                py = y + size * math.sin(angle)
                points.append(NSPoint(px, py))
            CGContextSetStrokeColorWithColor(context, color.rgb())
        ...
        def draw_data_streams(self, context, x, y):
            for stream in self.data_streams:
                if len(stream['trail_positions']) > 1:
                    for i, (trail_x, trail_y) in enumerate(stream['trail_positions']):
                        trail_alpha = int(stream['alpha'] * 255 * (i / len(stream['trail_positions'])) * self.intensity_boost)
                        ...
                        g = int(255)
                        b = int(200 + 55 * math.cos(color_phase))
                        CGContextSetRGBFillColor(context, r/255, g/255, b/255, trail_alpha/255.0)
                        size = stream['size'] * (1 + i * 0.1)
                        CGContextFillEllipseInRect(context, NSMakeRect(x + trail_x - size, y + trail_y - size, size * 2, size * 2))

        def draw_enhanced_orbit_particles(self, context, x, y):
            for particle in self.orbit_particles:
                px = x + particle['distance'] * math.cos(particle['angle'])
                py = y + particle['distance'] * math.sin(particle['angle'])
                alpha = int(255 * particle['current_alpha'])
                ...
                core_gradient.setColorAt(0.7, QColor(100, 200, 255, alpha//2))
                core_gradient.setColorAt(1, QColor(50, 150, 255, 0))
                CGContextSetRGBFillColor(context, 255, 255, 255, alpha/255.0)
                CGContextFillEllipseInRect(context, NSMakeRect(px - size, py - size, size * 2, size * 2))
                if self.intensity_boost > 1.2:
                    glow_size = size * 3
                    glow_gradient = QRadialGradient(px, py, glow_size)
                    glow_gradient.setColorAt(0, QColor(255, 255, 255, alpha//4))
                    glow_gradient.setColorAt(0.5, QColor(150, 255, 255, alpha//6))
                    glow_gradient.setColorAt(1, QColor(100, 200, 255, 0))
                    CGContextSetRGBFillColor(context, 255, 255, 255, alpha//4/255.0)
                    CGContextFillEllipseInRect(context, NSMakeRect(px - glow_size, py - glow_size, glow_size * 2, glow_size * 2))

        def draw_arc_particles(self, context, x, y):
            for arc in self.arc_particles:
                if arc['life'] < arc['max_life']:
                    start_x = x + arc['start_distance'] * math.cos(arc['start_angle'])
                    start_y = y + arc['start_distance'] * math.sin(arc['start_angle'])
                    ...
                    for i in range(3):
                        ...
                        mid_y = (start_y + end_y) / 2 + offset
                        arc_color = QColor(100 + i*50, 200 + i*25, 255, alpha//(i+1))
                        CGContextSetStrokeColorWithColor(context, arc_color.rgb())
                        CGContextSetLineWidth(context, 2)
                        CGContextMoveToPoint(context, start_x, start_y)
                        CGContextAddLineToPoint(context, mid_x, mid_y)
                        CGContextAddLineToPoint(context, end_x, end_y)
                        CGContextStrokePath(context)

        def draw_ultra_core(self, context, x, y):
            pulse = 0.9 + 0.1 * math.sin(self.animation_time * 4) * self.intensity_boost
            current_radius = self.radius * pulse
            for i in range(5):
                glow_radius = current_radius + 12 + i * 5
                ...
                if i == 0:
                    ...
                    glow_gradient.setColorAt(0.9, QColor(100, 200, 255, glow_alpha))
                    glow_gradient.setColorAt(1.0, QColor(50, 150, 255, 0))
                CGContextSetRGBFillColor(context, 255, 255, 255, glow_alpha/255.0)
                CGContextFillEllipseInRect(context, NSMakeRect(x - glow_radius, y - glow_radius, glow_radius * 2, glow_radius * 2))
            main_gradient = QRadialGradient(x - 15, y - 15, current_radius * 1.3)
            main_gradient.setColorAt(0, QColor(255, 255, 255, 220))
            main_gradient.setColorAt(0.1, QColor(240, 255, 255, 235))
            main_gradient.setColorAt(0.3, QColor(200, 240, 255, 245))
            ...
            border_alpha = int(150 + 100 * border_intensity)
            border_color = QColor(200, 255, 255, border_alpha)
            CGContextSetRGBFillColor(context, 255, 255, 255, 220/255.0)
            CGContextFillEllipseInRect(context, NSMakeRect(x - current_radius, y - current_radius, current_radius * 2, current_radius * 2))
            CGContextSetStrokeColorWithColor(context, border_color.rgb())
            CGContextSetLineWidth(context, 4)
            CGContextStrokeEllipseInRect(context, NSMakeRect(x - current_radius, y - current_radius, current_radius * 2, current_radius * 2))
            highlight1 = QRadialGradient(x - 18, y - 18, 40)
            highlight1.setColorAt(0, QColor(255, 255, 255, int(200 * self.intensity_boost)))
            highlight1.setColorAt(0.4, QColor(255, 255, 255, int(120 * self.intensity_boost)))
            highlight1.setColorAt(1, QColor(255, 255, 255, 0))
            CGContextSetRGBFillColor(context, 255, 255, 255, int(200 * self.intensity_boost) / 255.0)
            CGContextFillEllipseInRect(context, NSMakeRect(x - 30, y - 30, 50, 45))
            highlight2 = QRadialGradient(x - 25, y - 25, 25)
            highlight2.setColorAt(0, QColor(240, 255, 255, int(150 * self.intensity_boost)))
            highlight2.setColorAt(1, QColor(200, 255, 255, 0))
            CGContextSetRGBFillColor(context, 255, 255, 255, int(150 * self.intensity_boost) / 255.0)
            CGContextFillEllipseInRect(context, NSMakeRect(x - 35, y - 35, 30, 25))
            core_pulse = 0.6 + 0.5 * math.sin(self.animation_time * 8) * self.intensity_boost
            core_size = 8 * core_pulse
            core_gradient = QRadialGradient(x - 10, y - 10, core_size)
            core_gradient.setColorAt(0, QColor(255, 255, 255, int(255 * core_pulse)))
            core_gradient.setColorAt(0.5, QColor(240, 255, 255, int(200 * core_pulse)))
            core_gradient.setColorAt(1, QColor(200, 240, 255, int(150 * core_pulse)))
            CGContextSetRGBFillColor(context, 255, 255, 255, int(255 * core_pulse) / 255.0)
            CGContextFillEllipseInRect(context, NSMakeRect(x - 10 - core_size//2, y - 10 - core_size//2, core_size, core_size))

        def draw_inner_effects(self, context, x, y):
            for particle in self.energy_particles:
                px = x + particle['x']
                py = y + particle['y']
                dx = particle['x']
    ...
    class NativeAIPetWindow(NSWindow):
        def initWithContentRect_styleMask_backing_defer_(self, rect, style, backing, defer):
            ...
            if self:
                ...
                self.setMovableByWindowBackground_(YES)
                pet_view = NativeAIPetView.alloc().initWithFrame_(rect)
                self.setContentView_(pet_view)
            return self
        def canBecomeKeyWindow(self): return YES
        def acceptsFirstResponder(self): return YES

    class NativeAIPetApp(NSObject):
        def applicationDidFinishLaunching_(self, notification):
            window_rect = NSMakeRect(100, 100, 200, 200)
            self.window = NativeAIPetWindow.alloc().initWithContentRect_styleMask_backing_defer_(
                window_rect, NSWindowStyleMaskBorderless, NSBackingStoreBuffered, NO
            )
...
def main():
    import sys
    if sys.platform == "darwin" and "--native" in sys.argv:
        # macOS 原生窗口模式（需加 --native 参数）
        from AppKit import NSApplication
        app = NSApplication.sharedApplication()
        app_delegate = NativeAIPetApp.alloc().init()
